...
if (Sys.getenv("OPENSAFELY_BACKEND") %in% c("", "expectations")) {
    ...
    data_studydef_dummy <- read_feather(here("output", cohort, "extract", "input_treated.feather")) %>%
        # because date types are not returned consistently by cohort extractor
        mutate(across(ends_with("_date"), ~ as.Date(.))) %>%
        # because of a bug in cohort extractor -- remove once pulled new version
        mutate(patient_id = as.integer(patient_id))

    data_custom_dummy <- read_feather(here("lib", "dummydata", "dummy_treated_{cohort}.feather")) %>%
        mutate(
    ...
    data_extract <- read_feather(here("output", cohort, "extract", "input_treated.feather")) %>%
        # because date types are not returned consistently by cohort extractor
        mutate(across(ends_with("_date"), as.Date))
}
...
write_rds(data_treated_eligible, here("output", cohort, "treated", "data_treatedeligible.rds"), compress = "gz")
...
write_rds(data_flowchart, here("output", cohort, "treated", "flowchart_treatedeligible.rds"))
