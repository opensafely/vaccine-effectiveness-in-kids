...
data_control_matchinfo <- read_csv(fs::path(output_dir, glue("potential_matched_controls{matching_round}_{agegroup}.csv.gz")))
...
data_treated <- read_rds(fs::path(output_dir, glue("data_potential_matched{matching_round}_{agegroup}.rds"))) %>% filter(treated == 1L)
...
if (matching_round > 1) {
  data_matchstatusprevious <-
    read_rds(fs::path(output_dir, glue("data_matchstatus_allrounds{matching_roundprevious}_{agegroup}.rds"))) %>%
    filter(matched == 1L)
...
write_rds(data_matchstatus_allrounds, fs::path(output_dir, glue("data_matchstatus_allrounds{matching_round}_{agegroup}.rds")))
...
write_rds(data_match_actual, fs::path(output_dir, glue("data_match_actual{matching_round}_{agegroup}.rds")))
